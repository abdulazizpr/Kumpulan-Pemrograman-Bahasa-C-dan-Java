...
void addFirst(char nama[],char nim[],char gender[],list *L){
	...
	if(countElement(*L) < n){
		...
		strcpy((*L).data[baru].elmt.gender, gender);
...
void addAfter(int prev,char nama[],char nim[],char gender[],list *L){
...
void addLast(char nama[],char nim[],char gender[],list *L){
	
	if((*L).first == -1) addFirst(nama, nim, gender, L);		//proses jika list masih kosong
		...
		if(countElement(*L) < n){						//proses jika array belum penuh
			...
			addAfter(prev, nama, nim, gender, L);
		}
		else printf("sudah tidak dapat ditambahkan\n");	//proses jika array penuh
	}
...
void printElemen(list L){
	...
	printf("%s %s %s\n",L.data[elmt].elmt.nama,L.data[elmt].elmt.nim,L.data[elmt].elmt.gender);
